...
**Why phased, not a single big-bang rename**: the rename touches four independent risk layers — UI copy, internal identifiers, URL routes, and the DB schema — each with a very different blast radius. A single commit would conflate four reversibility profiles: copy is trivially reverted; identifier renames are compile-checked but fan out through every import; route renames affect bookmarks and external links and require a redirect layer; DB renames take a brief ACCESS EXCLUSIVE lock, require coordinated deploy, and must rewrite audit-event-value strings as a data migration. A phased approach lets each layer land on its own ticket, ship to users independently, and be rolled back independently. Phase 1 also surfaces any product-side objections to the new term (copy is visible; identifiers are not) before the irreversible schema work in later phases. The phases are ordered by ascending risk: Phase 1 UI copy → Phase 2 internal types/identifiers → Phase 3 routes with 308 redirects → Phase 4 DB table/column rename with coordinated deploy → Phase 5 docs sync.

**Why sentence case and British spelling**: sentence case matches the project-wide convention already established by MGT-099 (Event → Timetable) and reinforced by the UX Audit 2026-04-22 Suggested Improvement #8 button-casing pass. British spelling (`championship`, not `championships` with no "s" variant concern) matches the existing British spelling of `organisation` that was already the canonical form across the codebase (0 instances of American `organization` found in audit, 70+ instances of `organisation`). Staying British keeps a single dialect everywhere.

**Why leave audit-event-type values (`organisation.created` etc.) as data**: those strings are row values in `audit_log.event_type`, not schema. Rewriting them is a data migration — one that must be coordinated with code that matches on those values inside `OrgAuditLogView.tsx` (the `if (action === 'organisation.created')` branches drive which summary component renders). Changing the value without changing the matcher at the same moment breaks the audit log UI. That coupling lives in Phase 4 alongside the DB schema rename so both sides of the contract move together; Phase 1 leaves the data untouched and only rewrites the *labels* shown next to those values in the UI. Phase 4 will rewrite the existing rows (`UPDATE audit_log SET event_type = 'championship.created' WHERE event_type = 'organisation.created'`) and flip all matchers in the same coordinated deploy.

**Why function names and type names stay in Phase 1**: changing `createOrganisation` → `createChampionship` forces every call site (and every import statement) to change at the same time. That is a compile-checked change with no runtime risk, but it bloats Phase 1's diff, couples the copy change to a wider refactor, and makes rollback harder (reverting Phase 1 would require reverting N callers too). Keeping identifiers stable in Phase 1 means the copy ticket is a tiny, isolated, trivially reversible change; identifiers move wholesale in Phase 2.

**Scope boundary for Phase 1**:
- In scope: JSX text, `<h1>` / `<h2>` / `<p>` / `<span>` / `<th>` / `<title>` content, `aria-label`, `title` attributes, ConfirmDialog `description` props, server-action user-facing error message string literals (returned to the UI and rendered in `ERROR_BANNER`), email template HTML body text, legal document paragraphs (`/terms`, `/privacy`), `OrgAuditLogView` action-label map values (not keys).
- Out of scope (Phase 1): any identifier that compiles (function names, component names, TS types, file names, prop names, variable names, destructured values, imported symbol names), DB queries (`.from('organisations')`), audit-event-type values (data-layer strings), Sentry action tags, `OrganisationCreatedSummary` function name, code comments (not user-visible), route paths (`/admin/orgs/*`), public slug route (already entity-agnostic), existing `organisation.*` rows in `audit_log`.

**Rejected alternatives**:
1. *Big-bang rename in one commit* — rejected above. Conflates four reversibility profiles; a single failure in any of the four layers requires reverting all four.
2. *Phase 1 = copy + identifiers together* — rejected because it makes Phase 1 no longer trivially reversible (ripple through every caller) and mixes a compiler-verified refactor into a pure text change. Identifier moves deserve their own ticket so rollback is an isolated revert.
3. *Alias the old type names (`export type Organisation = Championship`) in Phase 2 for a deprecation window* — **resolved MGT-105 (Phase 2): hard delete, no aliases**. All callers are internal and the TypeScript compiler catches every miss; a same-commit delete keeps the rename a single atomic refactor with no stale identifier residue. Aliases would have added a second spelling every future contributor would have to choose between, defeating the point of the rename.
4. *Skip the legal pages (`/terms`, `/privacy`) in Phase 1 because they mention "organisation" in a legal-document context* — rejected. The user-facing term in legal copy is the entity name, and the entity is being renamed; leaving the legal pages on the old term creates an internal contradiction. Legal review is a separate concern (and nothing in this rename changes what the entity *is* — still a single-tenant container with owners, members, and events — only what it's called).
5. *Keep `Organisation` American spelling (`organization`) and only change to `championship`* — rejected because neither spelling is in use; the audit found zero American-spelling instances and 70+ British-spelling instances, so there is no spelling to standardise. The rename is organisation → championship, not British → American.

**Scope for future phases** (reference only, not decided in this DEC):
- **MGT-105 (Phase 2)** — internal types and identifier rename (`Organisation` → `Championship`, `ActiveOrg` → `ActiveChampionship`, `createOrganisation` → `createChampionship`, component file renames, helper file renames). Compile-checked; no user-visible change. **Completed 2026-04-23** — see PROJECT_STATUS MGT-105 entry for the full list of renamed types, files, server actions, helpers, and importers.
- **MGT-106 (Phase 3)** — admin URL segment rename `/admin/orgs` → `/admin/championships` with 308 redirect layer for existing bookmarks. Public slug route `/{slug}` unaffected. **Completed 2026-04-23** — folder renamed via `git mv`, seven route-string literals + six import paths + two comments updated, `next.config.mjs` `redirects()` block added with two 308 permanent rules (`/admin/orgs` and `/admin/orgs/:path*`). See PROJECT_STATUS MGT-106 entry.
- **MGT-107 (Phase 4)** — DB table rename + FK column rename + RLS policy recreate + audit-event-value data rewrite + regenerate `database.ts`. Coordinated deploy; highest risk; requires staging rehearsal + rollback drill. **Code complete locally 2026-04-24 (migration NOT yet applied to any Supabase project)** — scope landed: migration [supabase/migrations/20260424000000_mgt_107_rename_org_to_championship.sql](supabase/migrations/20260424000000_mgt_107_rename_org_to_championship.sql) (single tx: drop policies → drop XOR CHECK → rename tables `organisations`/`org_members`/`org_invites` → rename FK column `org_id → championship_id` on all 7 holder tables incl. `events`, `templates`, `audit_log`, `ai_extraction_log`, `notification_emails` → rename FK/unique/index identifiers → re-add XOR CHECK → backfill `audit_log.action` `'organisation.*' → 'championship.*'` → recreate every policy with new identifiers); hand-written `database.ts` re-keyed; `.from()` / nested join / `.eq('org_id')` / INSERT-UPDATE column keys swept across ~25 app files; audit-emitter strings in [src/app/admin/championships/actions.ts](src/app/admin/championships/actions.ts) flipped to `championship.created/updated/branding_updated`; `ChampionshipAuditLogView.tsx` fully de-orged with legacy branches deleted (backfill makes them dead code). **Scoped deferrals**: (1) `get_user_org_role(p_org_id)` function name and parameter retained — the name is internal and never surfaced in UI/API; renaming it alongside this rename would have required rebuilding ~70 policy bodies across 11 migrations, multiplying risk beyond the user-visible win. Deferred to a dedicated RLS-rebuild ticket. (2) `audit_log.detail` jsonb *payload keys* like `{ org_id: championship.id }` kept as-is — these are historical JSON keys inside stored blobs, not DB columns; renaming them would require a separate per-row jsonb rewrite with no user-facing benefit. (3) Cookie name `mgt-org-id` retained — a client session key; renaming would log every user out with no upside. (4) Sentry tag values like `action: 'createOrganisation.insertOrg'` retained — opaque observability labels. (5) `org_member.*` audit action strings retained — member/invite lifecycle events, distinct scope from `organisation.*`; left for a later targeted pass if needed. **State**: `npm run typecheck` / `npm test` 83/83 / `npm run build` all green; commit staged on `main`; migration **has not been applied** to staging or production Supabase. Next gate per plan: coordinated staging rehearsal → production cutover (snapshot → apply migration → merge → smoke).
- **MGT-108 (Phase 5)** — docs sync sweep across all `docs/*.md` (~70 references) + this DEC cross-linked to phase tickets.

**References**: UX Audit 2026-04-22 Suggested Improvement #8 (button casing pass, now extended into full terminology rename); MGT-099 (precedent: phased terminology unification for Event → Timetable); DEC-025 (audit_log schema includes `org_id` — noted as identifier to rename in Phase 4).
