...
3. *Alias the old type names (`export type Organisation = Championship`) in Phase 2 for a deprecation window* — deferred until Phase 2's plan is drafted. Grep confirms all callers are internal, so a same-commit delete is likely correct; flagging as open for Phase 2 review.
...
- **MGT-105 (Phase 2)** — internal types and identifier rename (`Organisation` → `Championship`, `ActiveOrg` → `ActiveChampionship`, `createOrganisation` → `createChampionship`, component file renames, helper file renames). Compile-checked; no user-visible change.
- **MGT-106 (Phase 3)** — admin URL segment rename `/admin/orgs` → `/admin/championships` with 308 redirect layer for existing bookmarks. Public slug route `/{slug}` unaffected.
