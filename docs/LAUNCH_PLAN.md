...
13. **AI timetable extraction** — Claude Vision document upload + OCR _(Phases A + B shipped MGT-069 / MGT-070 — real Claude Vision tool-use behind `MGT_AI_EXTRACTION_ENABLED`, `ai_extraction_log` observability, 20/org/24h rate limit, private `event-extractions` Storage archive — **Shipped (disabled in env)** pending `ANTHROPIC_API_KEY`, see MGT-071-BLOCKED. Review & correction contract formalised MGT-072 — pristine-snapshot preserved, `was_modified` flag logged via existing `ExtractionMeta`, no schema change. Admin visibility shipped MGT-073 — read-only Extraction log section on `/admin/orgs/settings` with status/date/search filters; queries `ai_extraction_log` directly, no schema change. UX polish shipped MGT-074 — empty-state redesign, dismissible mock-mode tip banner, status-pill dot + tooltip, "Updated ·" indicator, and "Clear filters" reset; presentation-only. Dev verification harness shipped MGT-075 — `npm run seed:extractions` / `npm run cleanup:extractions` populate and drop six marker-scoped `ai_extraction_log` rows covering every rendered branch so the panel can be exercised without a real `ANTHROPIC_API_KEY`; local/dev only, no production surface. Operational runbook for the harness documented MGT-076 — see **Extraction log — dev verification** in [QA_RUNBOOKS.md](QA_RUNBOOKS.md) for env vars, commands, expected panel states, and troubleshooting; per DEC-034, runbooks are the canonical home for operational QA flows. Triage chips + sort shipped MGT-077 — **Problems today** and **All problems** chips (`aria-pressed`, click-active-again-to-clear, mirroring the NotificationLogView idiom) plus a minimal **Sort order** select (Newest / Oldest first); **Clear filters** now also resets sort to newest. Presentation-only; no schema/RLS/API change.)_
