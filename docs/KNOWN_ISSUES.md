...
**Evidence (Sentry, org `lb42-ltd`, region `de.sentry.io`, project has only 3 unresolved issues total)**:
- `JAVASCRIPT-NEXTJS-R` — 4,966 events, first seen **2026-06-19T18:39:01Z** — `PGRST205: Could not find the table 'public.organisations' in the schema cache` — tag `action=publicChampionshipPage.resolveChampionship`. Root cause: `resolvePublicChampionshipBySlug()` on `origin/main`/`3c4220b` still queries `.from('organisations')`.
- `JAVASCRIPT-NEXTJS-S` — 1,494 events, first seen 2026-06-20T01:02:00Z — `PGRST200: Could not find a relationship between 'events' and 'organisations'` — tag `action=legacySlug.resolveEvent`. Root cause: `resolveLegacyEventPath()` in [src/app/(public)/[slug]/page.tsx](<../src/app/(public)/[slug]/page.tsx>) on `3c4220b` uses `.select('slug, organisations!inner(slug)')` and `.eq('org_id', championship.id)` against a schema that no longer has those names.
- Every event on both issues carries `release: 3c4220b997765d2cb965849e8a7c8b1ac01d4e8b`, `environment: production`, `handled: true`, and both issues fire within the same `trace_id` — one request to `/[slug]` or `/[slug]/[eventSlug]` produces both errors (metadata resolution + page render each hit the broken table). The failing transactions are real public pages (`Page Server Component /(public)/[slug]`, 1,604 events; `/(public)/[slug]/[eventSlug]`, 1,166 events) — not crawler/scanner noise, and not the `admin/events/[id]` or `admin/championships/settings` code paths (zero Sentry events on either).
...
2. Visit a real public championship page (`https://app.mygridtime.com/{a-real-championship-slug}`) and confirm it renders a list of events, not an empty/error state.
...
**Related**: see new DECISIONS.md entry (migrations that rename tables must never be applied to production ahead of the matching code deploy) and DEC-041 (original MGT-107 phased-rename plan, which specified staging rehearsal + rollback drill *before* production cutover — this incident is exactly the scenario that plan was designed to prevent).
...
- [src/components/admin/DayTab.tsx](../src/components/admin/DayTab.tsx) — strengthened header row typography from `text-[10px] text-gray-400` to `text-xs text-gray-600` (line 109); structure byte-identical (still `hidden md:flex`, still `grid-cols-[repeat(14,minmax(0,1fr))]`, still 5 spans `Title col-span-5` / `Start col-span-2` / `End col-span-2` / `Category col-span-2` / `Notes col-span-3`).
- [src/components/admin/EntryRow.tsx](../src/components/admin/EntryRow.tsx) — added `aria-label` attribute to each of the 5 inputs (lines 262 / 280 / 296 / 314 / 334) with values `"Title"` / `"Start time"` / `"End time"` / `"Category"` / `"Notes"`. `<label htmlFor>` pattern rejected — EntryRow is rendered many times, unique id wiring would require threading `_localId` into 5 more places; `aria-label` matches the project convention already used in `MemberManager.tsx`, `AuditLogView`, and EntryRow's own icon buttons.

**Out of scope**: mobile header row (mobile is `grid-cols-2` and 5 column headers do not map to 2 columns cleanly — mobile accessibility parity is satisfied by the new aria-labels since WAI-ARIA spec takes `aria-label` precedence over `placeholder` for accessible name regardless of viewport); `<label htmlFor>` refactor; any behaviour / validation / server-action / schema / RLS / migration change; placeholder rewrites; row-structure or FieldWrapper changes.

**Verification**: `npm run typecheck` clean; vitest 79/79 green; `npm run build` clean. Browser verification on `npm run dev` port 3000 as `mickbarlow@kiontechnology.co.uk` via `/api/auth/dev-session` (platform Staff, `MGT-060 Verify Org`) at `/admin/events/04438ed1-74fb-44d7-8dbc-3566912dd654`: at 1280×800 desktop, header row `font-size: 12px`, `color: rgb(75, 85, 99)`, 5 labels enumerated in order (Title / Start / End / Category / Notes) with correct col-spans; 10 sampled inputs across 2 entry rows report `aria-label` values `"Title"` / `"Start time"` / `"End time"` / `"Category"` / `"Notes"` repeating per row; screenshot confirms headers clearly readable above entries. At 375×812 mobile, header `getBoundingClientRect()` `{w: 0, h: 0}` confirms `hidden md:flex` still hides (no mobile layout change), `document.body.scrollWidth === 375` (no overflow regression vs MGT-094), `aria-label="Title"` still present on the first entry's title input (attributes are viewport-agnostic). Zero console errors / warnings at both widths. NVDA / VoiceOver / JAWS spot-check not exercised — `aria-label` is the standard accessibility hook and will be announced by any assistive technology.

**Status**: RESOLVED (2026-04-23). Deploy impact: code only (2 files, 1 className edit + 5 new attributes); no schema; no RLS; no env; no migration; no new DEC.

---

## MGT-099: Event/Timetable terminology inconsistency across UI — RESOLVED 2026-04-23

**Description**: UX Audit 2026-04-22 Suggested Improvement #3 (HIGH) and findings #25 / #30. The product used "Event" and "Timetable" interchangeably for the same underlying entity across its user-facing surfaces. `/admin` used "Event" ("Events" H1, "Create event" CTA, "Event details" editor card, "race event timetables" subtitle); `/my` used "Timetable" ("My Timetables" H1, "No timetables have been published yet.") but the header nav link still read "Manage events"; public pages used "Timetable" in prose ("Published timetables from this organisation.") but "Event" as the first-column header of both listing tables; notification emails mixed "timetable" + "event" + "schedule" inside a single sentence. The audit labelled this "the single loudest consistency problem."

**Fix**: UI-text-only pass. Primary term decided with user = **Timetable**; secondary terms ("schedule", "race event") removed. Internal vocabulary (DB tables `events`/`event_days`/`event_entries`, URL segments `/admin/events/[id]` / `/admin/events/new`, server-action names `createEvent` / `publishEvent` / `archiveEvent` / `duplicateEvent` / `saveAsTemplate`, audit-log action strings `event.published` / `timetable.updated`, TS types `Event` / `EventDay` / `TimetableEntry`, Resend template function names) all unchanged. Twelve files touched:
- [src/app/admin/page.tsx](../src/app/admin/page.tsx) — H1, subtitle, primary CTA, empty-state copy, empty-state CTA, load-error string.
- [src/app/admin/layout.tsx](../src/app/admin/layout.tsx) — admin header nav link text + `title` attribute.
- [src/app/admin/events/new/page.tsx](../src/app/admin/events/new/page.tsx) — breadcrumb root, breadcrumb current, H1, extract upload label, submit button (template branch keeps "Create from template").
- [src/components/admin/EventEditor.tsx](../src/components/admin/EventEditor.tsx) — validation-error string, card H2, both notification-empty paragraphs.
- [src/components/admin/ExtractionPreview.tsx](../src/components/admin/ExtractionPreview.tsx) — AI-extraction card H2, inline helper span, submit button.
- [src/app/my/layout.tsx](../src/app/my/layout.tsx) — elevated-role header nav link `Manage events` → `Manage timetables`.
- [src/app/(public)/page.tsx](../src/app/(public)/page.tsx) + [src/components/public/PublicOrgView.tsx](../src/components/public/PublicOrgView.tsx) — first-column table headers `Event` → `Timetable`.
- [src/app/admin/templates/page.tsx](../src/app/admin/templates/page.tsx), [src/app/admin/orgs/settings/page.tsx](../src/app/admin/orgs/settings/page.tsx), [src/app/admin/orgs/new/page.tsx](../src/app/admin/orgs/new/page.tsx) — breadcrumb root labels (parent-semantics on `orgs/settings` is audit item #29, separate ticket).
- [src/lib/resend/templates.ts](../src/lib/resend/templates.ts) — `eventPublishedHtml` / `eventPublishedText` body copy, `timetableUpdatedHtml` / `timetableUpdatedText` body copy (removes both "for this event" and "latest schedule before the event"), both HTML footer `notification list for this event` → `notification list for this timetable`. Email subjects already used "Timetable" and were left untouched.

**Out of scope**: route paths, server-action names, DB schema, audit-log action codes, `/my/upload/page.tsx` (already "Upload Timetable"), `Create from template` button branch (template is a distinct noun), breadcrumb parent-semantics on `/admin/orgs/settings` (audit item #29), in-popover role copy (audit item #27), button-casing sweep (Suggested Improvement #8), timetable row column headers (Suggested Improvement #6).

**Verification**: `npm run typecheck` clean; vitest 79/79 green (`src/lib/resend/templates.test.ts` pins titles / URLs / unsubscribe markers only — body copy not pinned, no test-file edits needed); `npm run build` clean. Browser verification via authenticated SSR grep against the running dev preview as `mickbarlow@kiontechnology.co.uk` (platform Staff, `MGT-060 Verify Org`): fetched `/admin`, `/my`, `/`, `/admin/events/new`, `/admin/events/[id]`, `/admin/orgs/settings`, `/admin/orgs/new`, `/admin/templates`, `/mgt-060-verify-org` and confirmed every expected new string rendered (`Timetables`, `Create timetable`, `No timetables found.`, `Manage timetables`, `New timetable`, `Timetable details`) and every legacy string absent. Final `src/` regex sweep for `>Event[s]?<`, `>Create event<`, `Manage events<`, `No events`, `race event`, `Upload a schedule`, `New event<`, `>Event details<` returned **zero matches**. Public-surface first-column table headers now render `<th>Timetable</th>` on both `/` and `/mgt-060-verify-org`. Validation-error string (`Timetable must have at least one day.`), notification-empty string (`No notification email addresses set for this timetable.`), and live Resend email send not exercised — they sit behind conditional render / external SMTP; code paths verified via compile + typecheck and will render the moment the underlying condition fires.

**Status**: RESOLVED (2026-04-23). Deploy impact: code only (12 files, UI + email string edits); no schema; no RLS; no env; no migration. No new DEC — terminology choice is operational, not architectural.

---

## MGT-098: MemberManager lets the acting user remove themselves — RESOLVED 2026-04-23

**Description**: UX Audit 2026-04-22 Critical #3 and Suggested Improvement #5 (HIGH). `MemberManager` rendered the per-row `Remove` button enabled on the acting user's own row. An owner could strand themselves out of the active org by mistake; the sole-owner self-remove case was caught server-side by the existing last-owner count check in `removeMember` but the client offered no pre-emptive warning and a multi-owner self-remove was not blocked anywhere.

**Fix**: Dual-layer guard, smallest possible change.
- **Client**: [src/components/admin/MemberManager.tsx](../src/components/admin/MemberManager.tsx) gained a required `currentUserId: string` prop threaded from [src/app/admin/orgs/settings/page.tsx](../src/app/admin/orgs/settings/page.tsx) through [src/app/admin/orgs/settings/SettingsPanels.tsx](../src/app/admin/orgs/settings/SettingsPanels.tsx). Inside the `members.map`, `const isSelf = member.user_id === currentUserId` gates the `Remove` button — `disabled={pending || isSelf}`, `aria-disabled={isSelf}`, title swap to `"You cannot remove yourself from this organisation."` when `isSelf`. `disabled:cursor-not-allowed` appended to the class list alongside the pre-existing `disabled:opacity-50`. `ConfirmDialog` never opens for the self row because `handleRemove` is unreachable when disabled.
- **Server**: [src/app/admin/orgs/actions.ts](../src/app/admin/orgs/actions.ts) `removeMember` now rejects with `'You cannot remove yourself from this organisation. Ask another owner to remove you, or switch organisations first.'` when `member.user_id === user.id`, placed between the member fetch and the existing last-owner check so the self-specific copy wins for sole-owner self-remove attempts. Reuses `user.id` already returned by `requireOwner()` — no new auth plumbing.

**Out of scope**: Self-role-demote from owner→editor when another owner exists does not remove access and is intentionally left unchanged; the existing last-owner guard in `updateMemberRole` remains the authoritative fallback for that path.

**Verification**: `npm run typecheck` clean; vitest 79/79 green; `npm run build` clean. Browser QA via the running dev preview on `/admin/orgs/settings` as `mickbarlow@kiontechnology.co.uk` (platform Staff, `MGT-060 Verify Org` — dev user is the only org member): self-row button reports `disabled === true`, `aria-disabled === "true"`, `title === "You cannot remove yourself from this organisation."`; clicking the disabled button produced zero `[role="dialog"]`/`.fixed.inset-0` overlays (ConfirmDialog stays closed); zero console logs/warnings; mobile 375×812 reports `document.body.scrollWidth === 359` (no overflow regression vs MGT-094 baseline); SSR fetch of the settings page confirms `<button disabled="" aria-disabled="true" … title="You cannot remove yourself from this organisation.">Remove</button>` renders from first paint (not a hydration-only effect). Other-member-row and sole-owner-self-remove branches verified by **code inspection only** — the dev session's only org has a single member, so a multi-member fixture is not safely reachable from dev without remote mutation (same limitation documented in MGT-090/091/094/095/097).

**Status**: RESOLVED (2026-04-23). Deploy impact: code only (4 files — `actions.ts` server guard, `MemberManager.tsx` client disable, `SettingsPanels.tsx` + `settings/page.tsx` prop threading); no schema; no RLS; no env; no migration. No new DEC — pure reuse of the last-owner guard pattern already established in `updateMemberRole`.

## MGT-097: /my lacks OrgSelector parity with /admin — RESOLVED 2026-04-23

**Description**: UX Audit 2026-04-22 Critical #6 and Suggested Improvement #4 (HIGH). The `/my` header only exposed the `MyGridTime` logo link, an optional `Manage events` link (elevated roles), and the `UserMenu` — multi-org consumers had no inline way to switch org context without opening the popover. Admin has had the `OrgSelector` in the header since MGT-084, and MGT-094 restored its mobile tap-target sizing; the audit called for the same switching affordance on `/my`.

**Fix**: Single-file reuse — [src/app/my/layout.tsx](../src/app/my/layout.tsx) imports the existing [`OrgSelector`](../src/components/admin/OrgSelector.tsx) and renders it in the header left-group (next to the logo) under the same gate used by admin (`userOrgs.length > 1 && activeOrg`). Left-group `<div>` gained `min-w-0` (so the selector's `max-w-[160px] truncate` is honoured in its flex parent); right-group `<div>` gained `min-w-0 flex-wrap gap-y-2` as defensive overflow safety matching MGT-094's admin pattern (adding a ≤160px control into the left-group reduces right-group slack on small viewports). No new component, no logic change, no server-action change — the existing `switchOrg()` action is reused unchanged; its `revalidatePath('/admin', 'layout')` is intentionally untouched because `router.refresh()` inside the client component already re-fetches `/my` after the cookie is set. The `UserMenu` popover org-switch rows (MGT-095) remain as a fallback when the selector is hidden (single-org user).

**Verification**: `npm run typecheck` clean; vitest 79/79 green; `npm run build` clean (after a `.next` wipe resolved the MGT-094/095/096 dev/build cache collision — classic pattern, not a code issue). Browser QA on `npm run dev` as `mickbarlow@kiontechnology.co.uk` (platform Staff, orgs `MGT-060 Verify Org` + `MGT QA Org B`) on `/my`: at 1440×900, `OrgSelector` present next to the logo, both orgs enumerated, active = `MGT-060 Verify Org`, `getBoundingClientRect()` = 28×144 at top 12 (byte-identical to `/admin` desktop rendering), `document.body.scrollWidth === 1440` (no overflow); at 375×812, same selector `height === 40` (WCAG 2.5.5 met, matches MGT-094 admin parity), `document.body.scrollWidth === document.documentElement.scrollWidth === 375` (no overflow regression vs audit baseline), badge wrapped cleanly to row 2 at 40×177; switch behaviour exercised at 1440 via native-setter + `change` event on the `<select>` — A→B→A round-trip both clean, `H1` stable at `My Timetables`, /my list re-rendered with Org B context on switch (distinct published events), zero console errors / warnings. Single-org and orgless branches verified by code inspection only — the dev session always has ≥1 org, same limitation documented in MGT-090/091/094/095. `BuildIdentityBadge` still visible on `/my` (MGT-093 unaffected — not touched).

**Status**: RESOLVED (2026-04-23). Deploy impact: code only (1 file — 1 new import + 1 new conditional element + 2 className edits); no schema; no env; no migration.

---

## MGT-096: Two separate Save buttons on event editor (details + timetable) with no unified commit — RESOLVED 2026-04-23

**Description**: UX Audit 2026-04-22 Critical #4. `/admin/events/[id]` exposed two independent primary Save buttons — "Save details" (metadata) on the Event details card and "Save timetable" (entries + deletions) inside the TimetableBuilder — each opening its own ReviewModal pass and each showing its own success banner. A user who edited both sections and clicked one button lost the other half ("did I save everything?"), commits interleaved two audit-log entries, and a partial-connectivity failure between the two clicks left the editor in divergent state. Audit Suggested Improvement #2 (HIGH) directed: "Unify event save into a single top-of-editor 'Save' commit covering both details and timetable; strip the two existing save buttons."

**Fix**: UI+orchestration only across 3 files (no server-action, type, validation, RLS, or migration change). [src/components/admin/EventActionsBar.tsx](../src/components/admin/EventActionsBar.tsx) gained optional `onSave` / `saving` / `saveSuccess` / `saveError` props and renders a single primary `Save changes` button (disabled when `!isDirty || saving`) plus one inline success/error slot in the status column. [src/components/admin/EventEditor.tsx](../src/components/admin/EventEditor.tsx) extended `reviewMode` to `'metadata' | 'timetable' | 'unified' | null`, added `runUnifiedSave()` (sequentially runs `performMetaSave` → `performTimetableSave`; aborts if meta fails; explicit error message on timetable-after-meta failure) and `handleUnifiedSave()` (hoists validation gates from the deleted `handleSaveMetadata` + `handleSaveTimetable`; opens unified review or direct-saves when only `notification_emails` changed); retargeted the details `<form onSubmit>` to `handleUnifiedSave` so Enter commits the full editor; deleted the "Save details" button row, the two per-section success banners, and both deprecated save handlers; `handleAcceptCard` / `handleRejectCard` now branch on `card.kind` so they work cross-mode; ReviewModal title is `Review changes` in unified mode; notify footer applicability extended to cover unified mode only when at least one timetable card is present (DEC-002 preserved). [src/components/admin/TimetableBuilder.tsx](../src/components/admin/TimetableBuilder.tsx) dropped `onSave` / `saving` / `saveError` / `saveSuccess` props and deleted the "Save timetable" button row. Lifecycle cluster (Publish / Unpublish / Archive / Duplicate / Save as Template) and day-label `onBlur` auto-save left untouched.

**Verification**: `npm run typecheck` clean; vitest 79/79 green; `npm run build` clean. Browser QA on `npm run dev` as `mickbarlow@kiontechnology.co.uk` at `/admin/events/04438ed1-74fb-44d7-8dbc-3566912dd654`: metadata-only dirty → Save opens `Review changes` modal with 1 meta card → Accept all & save closes modal, shows `Changes saved.` banner, clears dirty pill, disables Save; timetable-only dirty → unified modal lists 1 timetable card with `Review changes` title; timetable validation failure (cleared entry title) → modal does not open, status slot shows `Fix the highlighted timetable errors before saving.`. Per-card reject path and partial-failure message path not exercised — no safe way to force a server failure from dev; both paths are structurally preserved through the reused `performMetaSave` / `performTimetableSave` helpers.

**Status**: RESOLVED (2026-04-23). Deploy impact: code only (3 files, orchestration only); no schema; no env; no migration.
